...
import {CounterPresenter, CounterViewModel} from "./adapters/Presenter/CounterPresenter.ts";
import {Controller} from "./adapters/Controller/Controller.ts";
import {CounterAdapter, DecrementCounter} from "./application/DecrementCounter.ts";
import ReactCounter from "./outer/ReactCounter.tsx";
import {CounterStorage} from "./adapters/CounterStorage.ts";
import {Provider} from "./outer/Context.ts";
...
export class Main extends React.PureComponent<Props, State> {
    ...
    constructor(props: Props) {
        super(props);
        this._adapters = {
            counterStorage: new CounterStorage(0),
        ...
        return (
            <>
                <Provider value={{controller: this._controller}}>
                    {counterViewModel && <ReactCounter counterViewModel={counterViewModel}/>}
                </Provider>
            </>
        )
    }
